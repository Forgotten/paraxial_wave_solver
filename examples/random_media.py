...
def main():
  ...
  # Refractive index perturbation. The medium arrives as an argument rather
  # than a closure, so the compiled solver is reused across realizations.
  ...
  solver = pws.ParaxialWaveSolver(sim_config, solver_config, pml_config, delta_n_fn)
  psi_final, psi_history = solver.solve(psi_0, z_0=0.0, medium=delta_n)
